...
public class Commands extends RunCucumber {
    ...
    public static void scroll(int pixelsToScroll) {
        try {
            JavascriptExecutor jsExecutor = (JavascriptExecutor) getDriver();
            jsExecutor.executeScript("window.scrollBy(0, arguments[0]);", pixelsToScroll);
            System.out.println("Rolou a página verticalmente em " + pixelsToScroll + " pixels.");
        } catch (Exception error) {
            System.out.println("********** Aconteceu um erro ao tentar rolar a página.");
            System.out.println(error);
        }
    }
    ...
    public static void sleep(int milliseconds) {
        try {
            ...
            System.out.println("********** Ocorreu um erro durante o Thread.sleep()");
            e.printStackTrace();
        }
    }
}